...
class Fun(commands.Cog):
    ...
    @commands.command(description='Mostra il tempo rimanente per il grab e il drop')
    async def kcd(self, ctx: commands.Context):
        current_time = time.time()
        last_grab_time = self.last_grab.get(ctx.author.id, 0)
        last_drop_time = self.last_stamp.get(ctx.author.id, 0)
        grab_cooldown = 300  # Tempo in secondi tra due grab consecutivi (5 minuti)
        drop_cooldown = 600  # Tempo in secondi tra due drop consecutivi (10 minuti)

        grab_remaining = max(0, grab_cooldown - int(current_time - last_grab_time))
        drop_remaining = max(0, drop_cooldown - int(current_time - last_drop_time))

        await ctx.send(f"Tempo rimanente per il grab: {grab_remaining} secondi")
        await ctx.send(f"Tempo rimanente per il drop: {drop_remaining} secondi")
